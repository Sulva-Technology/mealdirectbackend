...
| `supabaseAuth` | The Supabase JWT token for authenticated endpoints. | *(JWT token retrieved from authentication)* |
| `operationsToken` | The internal operations token used for admin/infrastructure endpoints. | *(Defined in `.env` as `INTERNAL_OPERATIONS_TOKEN`)* |
