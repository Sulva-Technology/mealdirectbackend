...
-- Per-item customization (spoon counts, custom food type/protein, custom selections, drinks)
-- is stored as a flexible jsonb object snapshot alongside the priced order item. Order-level
-- special instructions (delivery note) are stored on the order. Both are immutable after the
-- order leaves pending_payment via the existing order_items / orders protection triggers.

alter table public.order_items
  add column customization jsonb not null default '{}'::jsonb;

alter table public.order_items
  add constraint order_items_customization_object check (jsonb_typeof(customization) = 'object');

comment on column public.order_items.customization is 'Customer per-item customization snapshot (spoonsCount, customFoodType, customProtein, customFoodSelections, drinks, etc.). Flexible jsonb object preserved at order time.';

alter table public.orders
  add column special_instructions text;

alter table public.orders
  add constraint orders_special_instructions_length check (special_instructions is null or length(special_instructions) <= 1000);

comment on column public.orders.special_instructions is 'Optional customer delivery note / special instructions for the whole order.';

-- Re-create create_pending_order_and_reserve_inventory adding the order-level special
-- instructions parameter and persisting per-item customization carried inside p_items.
-- The added parameter changes the signature, so drop the prior 11-argument overload first.
drop function if exists public.create_pending_order_and_reserve_inventory(
  uuid, uuid, uuid, date, uuid, uuid, public.delivery_mode, jsonb, text, text, text
);
...
comment on function public.create_pending_order_and_reserve_inventory(uuid, uuid, uuid, date, uuid, uuid, public.delivery_mode, jsonb, text, text, text, text) is 'Creates a pending-payment order, reserves inventory under row locks, enforces cutoff and spoon limits, applies the zone delivery fee and an optional promotion code, persists per-item customization and order special instructions, and records idempotency.';
