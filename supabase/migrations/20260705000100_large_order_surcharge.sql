...
--   Orders whose total exceeds the standard cap (p_max_order_total_kobo, ₦2490) are no longer
--   hard-rejected. Instead the customer may opt in (p_accept_large_order) and pay a surcharge of
--   (total * p_large_order_surcharge_bps) + p_large_order_surcharge_flat_kobo on top of the total.
--   Defaults 150 bps (1.5%) + ₦100, mirroring Paystack's own fee tier. When the customer has not
--   accepted, the order is still rejected. The surcharge amount is persisted for reporting.

alter table public.orders
  add column if not exists large_order_surcharge_kobo integer not null default 0
  check (large_order_surcharge_kobo >= 0);

comment on column public.orders.large_order_surcharge_kobo is 'Large-order surcharge in kobo (1.5% + ₦100 by default) applied when the order total exceeds the standard cap and the customer accepted it. Included in total_kobo.';

-- Recreate create_pending_order_and_reserve_inventory: identical to
-- 20260627000100_admin_fee_config_order_cap.sql except it accepts the surcharge parameters,
-- computes/persists the surcharge, and only rejects an over-cap order when it was not accepted.
drop function if exists public.create_pending_order_and_reserve_inventory(
  uuid, uuid, uuid, date, uuid, uuid, public.delivery_mode, jsonb, text, text, text, text, integer, integer
);
