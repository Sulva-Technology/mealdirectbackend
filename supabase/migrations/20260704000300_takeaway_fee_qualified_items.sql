...
declare
  ...
  v_requested_service_fee integer := 0;
...
begin
  ...
  v_requested_service_fee := greatest(coalesce(p_service_fee_kobo, 0), 0);
  ...
  if v_spoon_units > 0 then
    v_service_fee := v_requested_service_fee;
  end if;
...
comment on function public.create_pending_order_and_reserve_inventory(uuid, uuid, uuid, date, uuid, uuid, public.delivery_mode, jsonb, text, text, text, text, integer, integer) is 'Creates a pending-payment order, reserves inventory under row locks, enforces cutoff/spoon/max-total limits, applies the zone delivery fee, flat per-order service fee only when at least one item counts toward takeaway, and an optional promotion code, persists per-item customization and order special instructions, and records idempotency.';
