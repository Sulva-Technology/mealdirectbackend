...
function p(children, opts = {}) {
  return new Paragraph({
    children: Array.isArray(children) ? children : [txt(String(children))],
    ...opts
  });
}
function mono(text) {
...
function h3(text) {
  return new Paragraph({ heading: HeadingLevel.HEADING_3, children: [txt(text)] });
}
...
function cell(content, { w, fill, header = false } = {}) {
  const children = Array.isArray(content)
    ...
    : [
        new Paragraph({
          children:
            typeof content === 'string' ? [txt(content, header ? { bold: true } : {})] : [content]
        })
      ];
...
function spacer() {
  return new Paragraph({ children: [txt('')], spacing: { after: 60 } });
}
...
children.push(
  new Paragraph({
    ...
    spacing: { after: 40 },
    children: [
      txt('Request input, validation rules, and response schemas', { italics: true, size: 24 })
    ]
  })
);
children.push(
  new Paragraph({
    ...
    spacing: { before: 600 },
    children: [
      txt('Base path: ', { size: 22 }),
      mono('/v1/admin'),
      txt('   •   Generated 2026-06-23', { size: 22 })
    ]
  })
);
...
children.push(
  p([
    txt('Every admin route requires a valid Supabase JWT in the '),
...
children.push(
  p([
    txt('Controller-level role gate: '),
    mono("@RequireRoles('campus_admin', 'super_admin')"),
    txt('.')
  ])
);
children.push(
  paramsTable([
    [
      ...
      'n/a',
      'yes',
      'campus_admin → scoped to own campusId; super_admin → global, may pass any campusId.'
    ],
    ...
    [
      ...
      'n/a',
      'n/a',
      'Admin role required; campus_admin requesting another campus; super-admin-only routes.'
    ]
  ])
);
children.push(
  note('Super-admin-only routes: user suspend/activate, all /admin-memberships routes.')
);
...
children.push(
  fieldsTable([
    ...
    [
      { code: 'nextCursor' },
      'string (optional)',
      'Opaque base64url cursor; omitted when hasMore is false. Note: cursor not emitted by current admin list queries (offset-style limit+1).'
    ]
  ])
);
...
children.push(
  fieldsTable([
    [
      { code: 'date' },
      'string',
      'ISO calendar date, pattern ^\\d{4}-\\d{2}-\\d{2}$ (e.g. 2026-06-23).'
    ],
...
function emit(opts) {
  ...
  if (opts.roles)
    children.push(p([txt('Access: ', { bold: true, size: 20 }), txt(opts.roles, { size: 20 })]));
  if (opts.params && opts.params.length) {
    children.push(label('Path parameters'));
    children.push(paramsTable(opts.params));
  }
  if (opts.query && opts.query.length) {
    children.push(label('Query parameters'));
    children.push(paramsTable(opts.query));
  }
  if (opts.body && opts.body.length) {
    children.push(label('Request body (application/json)'));
    children.push(paramsTable(opts.body));
  }
...
emit({
  method: 'GET',
  route: '/session',
  title: 'Authenticated admin session and scope.',
  ...
  resFields: [
    ...
    [
      { code: 'scopes' },
      'array<string>',
      "['admin:global'] for super_admin; ['admin:campus:<id>'] for campus_admin."
    ]
  ]
});
...
emit({
  method: 'GET',
  route: '/dashboard',
  title: 'Admin operational dashboard for a service date.',
  roles: 'campus_admin, super_admin',
  query: [P.campusId, ['date', 'string', 'no', 'Service date (YYYY-MM-DD). Defaults to today.']],
  resFields: [
...
emit({
  method: 'GET',
  route: '/orders',
  title: 'List orders (filtered, paginated).',
  query: [
    P.cursor,
    P.limit,
    P.campusId,
    [
      ...
      'string',
      'no',
      'One of: accepted, administratively_completed, cancelled, confirmed, delivered, expired, out_for_delivery, paid, pending_payment, preparing, ready, refunded.'
    ],
...
emit({
  ...
  body: [
    [
      ...
      'string',
      'no',
      'Max 500 chars. Defaults to "Cancelled by admin." Drives a status transition to cancelled.'
    ]
  ],
...
emit({
  method: 'POST',
  route: '/orders/:orderId/status-transition',
...
emit({
  ...
  title: 'Get a single batch (with assignment).',
  params: [['batchId', 'uuid', 'yes', 'Batch id.']],
  resFields: batchGetFields,
  notes: ['404 NOT_FOUND if not found / outside scope.']
});

emit({
  method: 'POST',
  route: '/batches/:batchId/close',
  title: 'Close a batch.',
  params: [['batchId', 'uuid', 'yes', 'Batch id.']],
  body: [],
  resType: '200 OK, success envelope { data } (batch record)',
  resFields: batchGetFields
});

emit({
  method: 'POST',
  route: '/batches/:batchId/assign-rider',
  title: 'Assign a rider to a batch.',
  params: [['batchId', 'uuid', 'yes', 'Batch id.']],
...
emit({
  method: 'POST',
  route: '/batches/:batchId/assign-vendor-delivery',
  title: 'Assign vendor self-delivery to a batch.',
  params: [['batchId', 'uuid', 'yes', 'Batch id.']],
...
emit({
  method: 'POST',
  route: '/batches/:batchId/reassign-rider',
  title: 'Reassign batch to a different rider.',
  params: [['batchId', 'uuid', 'yes', 'Batch id.']],
...
emit({
  method: 'POST',
  route: '/batches/:batchId/cancel-assignment',
  title: 'Cancel the current batch assignment.',
  params: [['batchId', 'uuid', 'yes', 'Batch id.']],
  body: [],
  resType: '200 OK, success envelope { data } (batch record)',
  resFields: batchGetFields
...
emit({
  ...
  resFields: vendorGetFields
});

emit({
  method: 'PATCH',
  route: '/vendors/:vendorId',
  title: 'Update vendor fields.',
  params: [['vendorId', 'uuid', 'yes', 'Vendor id.']],
  body: [
  ...
  resType: '200 OK, success envelope { data } (vendor record)',
  resFields: vendorGetFields
});
...
emit({
  ...
  title: 'Approve a vendor (status=approved, active=true).',
  params: [['vendorId', 'uuid', 'yes', 'Vendor id.']],
  body: [],
  resType: '200 OK, success envelope { data } (vendor record)',
  resFields: vendorGetFields
...
emit({
  method: 'POST',
  route: '/vendors/:vendorId/suspend',
  title: 'Suspend a vendor (status=suspended).',
  params: [['vendorId', 'uuid', 'yes', 'Vendor id.']],
  body: [],
  resType: '200 OK, success envelope { data } (vendor record)',
  resFields: vendorGetFields
...
emit({
  method: 'POST',
  route: '/vendors/:vendorId/activate',
  title: 'Activate a vendor (alias of approve).',
  params: [['vendorId', 'uuid', 'yes', 'Vendor id.']],
  body: [],
  resType: '200 OK, success envelope { data } (vendor record)',
  resFields: vendorGetFields
...
emit({
  ...
  title: 'Add or upsert a vendor user.',
  params: [['vendorId', 'uuid', 'yes', 'Vendor id.']],
  body: [
...
emit({
  method: 'GET',
  route: '/vendors/:vendorId/performance',
...
const riderGetFields = riderFields.concat([
  [{ code: 'updatedAt' }, 'string', 'Updated timestamp.']
]);
...
emit({
  ...
  resFields: riderGetFields
});

emit({
  method: 'GET',
  route: '/riders/:riderId/assignments',
  title: 'List a rider’s delivery assignments (max 100).',
  params: [['riderId', 'uuid', 'yes', 'Rider id.']],
...
emit({
  method: 'GET',
  route: '/riders/:riderId/settlements',
  title: 'List a rider’s settlements (max 100).',
  params: [['riderId', 'uuid', 'yes', 'Rider id.']],
...
emit({
  method: 'POST',
  route: '/riders/:riderId/verify',
  title: 'Verify a rider (status=verified, active=true).',
  params: [['riderId', 'uuid', 'yes', 'Rider id.']],
  body: [],
  resType: '200 OK, success envelope { data } (rider record)',
  resFields: riderGetFields
});

emit({
  method: 'POST',
  route: '/riders/:riderId/suspend',
  title: 'Suspend a rider (status=suspended).',
  params: [['riderId', 'uuid', 'yes', 'Rider id.']],
  body: [],
  resType: '200 OK, success envelope { data } (rider record)',
  resFields: riderGetFields
...
emit({
  method: 'POST',
  route: '/riders/:riderId/activate',
  title: 'Activate a rider (alias of verify).',
  params: [['riderId', 'uuid', 'yes', 'Rider id.']],
  body: [],
  resType: '200 OK, success envelope { data } (rider record)',
  resFields: riderGetFields
...
emit({
  method: 'GET',
  route: '/inventory',
  title: 'List menu item inventory (max 100).',
  query: [
    P.campusId,
    ['date', 'string', 'no', 'Service date (YYYY-MM-DD).'],
    ['slotId', 'uuid', 'no', 'Filter by delivery slot.'],
...
emit({
  method: 'POST',
  route: '/inventory/:inventoryId/adjustments',
  title: 'Record an inventory adjustment.',
  ...
  resType: '201 Created, success envelope { data }',
  resFields: [
    [{ code: 'adjustmentId' }, 'string', 'Created adjustment id (record_inventory_adjustment).']
  ]
});
...
emit({
  ...
  title: 'Get an escalation.',
  params: [['id', 'uuid', 'yes', 'Escalation id.']],
  resFields: escGetFields
});

emit({
  method: 'POST',
  route: '/escalations/:id/assign',
  title: 'Assign escalation to an admin (status→investigating).',
  params: [['id', 'uuid', 'yes', 'Escalation id.']],
  body: [['adminUserId', 'uuid', 'yes', 'Admin user to assign.']],
  resType: '200 OK, success envelope { data } (escalation record)',
  resFields: escGetFields
});

emit({
  method: 'POST',
  route: '/escalations/:id/request-evidence',
  title: 'Request evidence (status→investigating).',
  params: [['id', 'uuid', 'yes', 'Escalation id.']],
  body: [],
  resType: '200 OK, success envelope { data } (escalation record)',
  resFields: escGetFields
...
emit({
  method: 'POST',
  route: '/escalations/:id/resolve',
  title: 'Resolve escalation (status→resolved).',
  params: [['id', 'uuid', 'yes', 'Escalation id.']],
...
emit({
  method: 'POST',
  route: '/escalations/:id/refunds',
  title: 'Mark escalation refunded (status→resolved).',
  params: [['id', 'uuid', 'yes', 'Escalation id.']],
  body: [],
  resType: '201 Created, success envelope { data } (escalation record)',
  resFields: escGetFields
});
...
emit({
  ...
  resType: '200 OK, list envelope { data[], pagination }',
  resFields: settlementListFields
});

emit({
  method: 'POST',
  route: '/settlements/preview',
  title: 'Preview a settlement (no write).',
  body: [
  ...
  notes: [
    'Rider preview returns beneficiaryType/Id/Date, deliveryEarningsKobo, estimatedPayableKobo.',
...
emit({
  ...
  title: 'Generate (persist) a settlement.',
  body: [
    ['beneficiaryType', 'string', 'yes', 'rider or vendor.'],
    ['beneficiaryId', 'uuid', 'yes', 'Vendor or rider id.'],
    ['settlementDate', 'string', 'yes', 'Date (YYYY-MM-DD).']
  ...
  resType: '201 Created, success envelope { data }',
  resFields: [
    [{ code: 'settlementId' }, 'string', 'Created settlement id (produce_*_daily_settlement).']
  ]
});
...
emit({
  ...
  resFields: settlementGetFields
});

emit({
  method: 'POST',
  route: '/settlements/:id/approve',
  title: 'Approve a settlement (status=approved).',
  params: [['id', 'uuid', 'yes', 'Settlement id.']],
  body: [],
  resType: '200 OK, success envelope { data } (settlement record)',
  resFields: settlementGetFields
});

emit({
  method: 'POST',
  route: '/settlements/:id/mark-paid',
...
emit({
  method: 'POST',
  route: '/settlements/:id/adjustments',
  title: 'Add a settlement adjustment line.',
  params: [['id', 'uuid', 'yes', 'Settlement id.']],
  body: [
    ['amountKobo', 'number (integer)', 'yes', 'Signed adjustment amount in kobo.'],
    ['description', 'string', 'yes', '3–200 chars.']
  ],
  resType: '201 Created, success envelope { data } (settlement record)',
  resFields: settlementGetFields
});
...
emit({
  ...
  route: '/users/:userId',
  title: 'Get a user.',
  params: [['userId', 'uuid', 'yes', 'User id.']],
  resFields: userFields
});
...
emit({
  ...
  title: 'Suspend a user. (super_admin only)',
  roles: 'super_admin only',
  params: [['userId', 'uuid', 'yes', 'User id.']],
  body: [],
  resType: '200 OK, success envelope { data } (user record)',
  resFields: userFields
});
...
emit({
  ...
  title: 'Activate a user. (super_admin only)',
  roles: 'super_admin only',
  params: [['userId', 'uuid', 'yes', 'User id.']],
  body: [],
  resType: '200 OK, success envelope { data } (user record)',
...
emit({
  ...
  title: 'List all admin memberships.',
  roles: 'super_admin only',
  resType: '200 OK, list envelope { data[], pagination } (hasMore=false)',
...
emit({
  method: 'POST',
  route: '/admin-memberships',
  title: 'Create an admin membership.',
  roles: 'super_admin only',
  body: [
  ...
  resType: '201 Created, success envelope { data }',
  resFields: membershipFields
});

emit({
  method: 'POST',
  route: '/admin-memberships/:id/revoke',
  title: 'Revoke a membership (active=false).',
  roles: 'super_admin only',
  params: [['id', 'uuid', 'yes', 'Membership id.']],
  body: [],
  resType: '200 OK, success envelope { data }',
  resFields: membershipFields
});
...
emit({
  method: 'POST',
  route: '/admin-memberships/:id/activate',
  title: 'Re-activate a membership (active=true).',
  roles: 'super_admin only',
  params: [['id', 'uuid', 'yes', 'Membership id.']],
  body: [],
  resType: '200 OK, success envelope { data }',
...
emit({
  method: 'GET',
  route: '/analytics',
  title: 'Aggregate order analytics.',
  query: [
    P.campusId,
